...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    console.log('🔧 Login logo upload debug access - bypassing authentication temporarily');
    const userToken = {
    ...
    console.log(`🎨 Upload Login Logo Request:`, {
    ...
    const loginLogoFile = files.loginLogo;

    if (!loginLogoFile) {
      console.log('❌ Login logo file not found in parsed data. Available files:', Object.keys(files));
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'File logo di login non trovato' })
      };
    }

    // Validate file type
    const allowedTypes = ['image/png', 'image/jpeg', 'image/jpg'];
    if (!allowedTypes.includes(loginLogoFile.contentType)) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ 
    ...
    if (loginLogoFile.data.length > maxSize) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ 
          error: 'File troppo grande. La dimensione massima è 2MB.',
          fileSize: loginLogoFile.data.length,
          maxSize: maxSize
        })
      };
    }

    // Generate unique filename with login prefix
    const fileExtension = loginLogoFile.contentType === 'image/png' ? 'png' : 'jpg';
    const uniqueId = randomBytes(16).toString('hex');
    const filename = `login-logo-${uniqueId}.${fileExtension}`;
    ...
    // Clean up old login logo files (find and delete previous login logo files)
    try {
      const fs = require('fs');
      const files = fs.readdirSync(uploadsDir);
      for (const file of files) {
        if (file.startsWith('login-logo-') && file !== filename) {
          const oldFilePath = join(uploadsDir, file);
          fs.unlinkSync(oldFilePath);
          console.log(`Deleted old login logo file: ${file}`);
        }
      }
    } catch (error) {
      console.log('No old login logo files to clean up or cleanup failed:', error);
    }

    // Save new file
    try {
      const fs = require('fs');
      fs.writeFileSync(filePath, loginLogoFile.data);
    } catch (error) {
      console.error('Failed to save login logo file:', error);
      return {
        statusCode: 500,
        headers,
    ...
    console.log('Login logo uploaded:', {
      timestamp: new Date().toISOString(),
      action: 'login_logo_uploaded',
      userId: userToken.userId,
      userEmail: userToken.email,
      filename: filename,
      path: relativePath,
      size: loginLogoFile.data.length,
      contentType: loginLogoFile.contentType
    });
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        message: 'Logo di login caricato con successo',
        data: {
          filename: filename,
          path: relativePath,
